...
<xsl:stylesheet
	...
	<xsl:template
		match='group[starts-with(./@description, "org.argouml.uml.diagram.ui.FigAssociationClassTee")]' />

	<!-- Remove href attribute from any FigEdgeNote groups (issue 4021) -->
	<xsl:template
		match='group[./@description="org.argouml.uml.diagram.static_structure.ui.FigEdgeNote"]'>
	...
	<xsl:template
		match='pgml/group[count(./@href) = 0 and ./@description != "org.argouml.uml.diagram.static_structure.ui.FigEdgeNote"]' />


	<!-- Remove top level groups that have no positional data (issue 4228) -->
	<xsl:template
		match='pgml/group[contains(@description, "[]")]'>
	</xsl:template>
