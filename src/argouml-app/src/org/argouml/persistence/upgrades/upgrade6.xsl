...
<xsl:stylesheet
	...
	<!-- Write <member> tags within <members> ordered by profile, xmi, pgml, 
		todo -->
	<xsl:template match='member[1]'>
		<members>
			...
			<xsl:for-each
				select='/uml/argo/member[./@type = "pgml"]'>
				<member>
					<xsl:copy-of select="@*" />
				</member>
			</xsl:for-each>
			<xsl:for-each
				select='/uml/argo/member[./@type = "todo"]'>
				<member>
					<xsl:copy-of select="@*" />
				</member>
			</xsl:for-each>
			...
			<xsl:for-each
				select='/uml/argo/member[./@type = "profile" and ./@type = "xmi" and ./@type = "pgml" and ./@type = "todo"]'>
				<member>
					<xsl:copy-of select="@*" />
				</member>
			</xsl:for-each>
	...
	<xsl:template
		match="/uml/pgml/group[./@description='org.argouml.uml.diagram.ui.FigEdgeAssociationClass']/private[contains(.,'sourcePortFig=&quot;tee_')]">
		<private>
			<xsl:value-of
				select="substring-before(.,'sourcePortFig=&quot;tee_')" />
		</private>
	</xsl:template>

	<!-- Remove any groups containing LAYER_NULL issue 5247 -->
	<xsl:template
		match="/uml/pgml/group[contains(./private,'LAYER_NULL')]">
	</xsl:template>
